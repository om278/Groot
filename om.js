...
if (text.includes("open WhatsApp")) {
    ...
    if (text.includes("what's the time")) {
	 ...
	 if (text.includes('play a song') || text.includes('music') || text.includes('song')) {
      p = document.createElement("p");
      p.classList.add("replay");
      p.innerText = "showing.....";
      texts.appendChild(p);
      console.log("showing.....");
      window.open(`https://www.youtube.com/search?q=${text.replace(" ", "+")}`, "_blank");
    }
	 ...
	 if (text.includes('wikipedia')) {
      ...
      texts.appendChild(p);
      console.log("showing.....");
      window.open(`https://en.wikipedia.org/wiki/${text.replace("wikipedia", "")}`, "_blank");
    }
